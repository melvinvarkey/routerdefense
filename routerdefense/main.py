...
parser = argparse.ArgumentParser(description='Cisco IOS configuration security assessment tool.', epilog='RouterDefense 2013.12')
parser.add_argument('--iosconfig', default='', help='Cisco IOS configuration file.',required=True)
parser.add_argument('--iosplatform', default='both', help='Cisco IOS platform: router, switch or both (multiple layers like Cat4k or 6k).')
parser.add_argument('--ip4inbound', default='', help='Authorized nodes to do VTY inbound sessions (Telnet, SSH).\nPossibles values examples (comma-separated) : 192.168.100.0/24, 192.168.1.1')
parser.add_argument('--ip4outbound', default='', help='Authorized nodes to communicate with the device in the outbound direction: SNMP, SYSLOG.\nPossibles values examples (comma-separated) : 192.168.100.0/24, 192.168.1.1')
parser.add_argument("--reportformat", default='stdout', help = "Report format support: stdout, csv, html, pdf.")
parser.add_argument("--reportfilename", default='report-routerdefense.pdf', help = "Report filename.")
args = parser.parse_args()
# argparse arguments parsing
__builtin__.deviceType = args.iosplatform
IP4inbound             = args.ip4inbound
IP4outbound            = args.ip4outbound
__builtin__.outputType = args.reportformat
__builtin__.outputFile = args.reportfilename
if len(IP4inbound) > 0:
